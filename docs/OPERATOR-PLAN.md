...
Final follow-up acceptance: commit `053ce14` passed 50 native open/close cycles and the full 600-second machine soak (198 observations; last timed observation 597.30 seconds, followed by the final interval and successful teardown). Evidence is `/tmp/tactical-shared-chrome-native.json`. No helper duplication, stale sequence, surviving helper or configured resource-limit failure occurred. Whole-shell peak growth was 84.73 MiB during lifecycle and 115.20 MiB during soak, below the configured 128 MiB ceiling; FD and child growth were zero. This is measured containment, not a zero-growth/leak-free claim, and does not explain the two earlier lost-instance runs. The runner now requests per-invocation privacy without modifying the saved preference.

A caption-only host-theme update exposed stale graph label geometry when observations did not change (including frozen data). An isolated Qt regression failed before adding the missing caption-token signal handler, then passed at normal and 1.5× Qt scaling. The fix preserves default font sizes and node positions, and remains inactive while the field is inactive. Full automated verification is 114 Python tests (one filesystem capability skip), 91 JavaScript tests, Qt controller/surface coverage, source/manifest validation and native import lint. This small follow-up was applied only after the completed soak, so it could not invalidate that run. Native theme/high-DPI/hotplug matrices remain separate gates.

The final caption-follow-up source additionally passed a clean-shell native five-instrument smoke run (`/tmp/tactical-final-caption-smoke.json`) after all automated gates; the 16-case Qt surface suite passed at 1.5×. The smoke is not another ten-minute soak. The acceptance helper and displayed surfaces were closed, and the original Machine instrument / saved non-private preference restored after testing.

Earlier refinement verification: 111 Python tests (one filesystem capability skip), 91 JavaScript tests, native lint and source/manifest validation passed. Native large-type component coverage also caught and fixed briefing-title overlap; the title now wraps within its allotted width. The real PipeWire silent-stream integration passed (one stream, two links). Forty samples for each of five instruments measured p95 collection times of 42–54 ms and peak sampler RSS below 33 MiB on this host, while the native machine soak was active. These measurements do not establish GPU frame time.
