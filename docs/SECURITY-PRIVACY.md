...
## Operator session retention

Operator briefing history is bounded in memory to 120 lifecycle records over five minutes, with a 2,048-key deduplication ceiling and at most eight pins. A baseline stores aggregate scalars and quality/timestamps, not a raw frame. Closing clears these references. Identity-bearing names may remain in memory while open, but privacy mode applies the field's aliases to retained activity and pins and uses an explicit redacted report projection. **Copy report** is an opt-in clipboard write: closing the plugin does not erase the desktop clipboard or a clipboard manager's history. Review reports before sharing them, even when identities are masked.
