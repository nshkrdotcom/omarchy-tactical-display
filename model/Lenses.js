function controls(instrument,state,settings) {
    ...
    if(instrument==='processes')return [{key:'emphasis',label:'Emphasis: '+(f.emphasis||'cpu'),values:['cpu','memory','threads','network','io'],hint:'E / Resource contribution; not a process-management action'}];
...
function settingsRows(){return [
    ...
    {key:'animation',label:'Motion',choices:['reduced','normal','vivid'],hint:'Finite acquisition and mode transitions only. No decorative traffic animation.'},
    ...
    {key:'privacy',label:'Screen-share privacy',choices:[true,false],hint:'Hides identities, search text and the underlying desktop. Not a security boundary.'},
